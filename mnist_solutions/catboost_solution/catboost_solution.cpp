...
int main(int argc, char* argv[])
{
    if(argc != 3){
        ...
        cout<<result<<"  "<<"path/to/test.csv  path/to/model.cbm"<<endl;
        cout<<"EXAMPLE:"<<endl;
        cout<<"logistic_regression_solution ../test.csv ../model.cbm"<<endl;
        std::terminate();
    }
    const std::string model_file(argv[2]);
    const std::string test_file(argv[1]);

    auto predictor = CatboostClassifier{model_file};
    auto features = CatboostClassifier::features_t{};
    double y_pred_expected = 0.0;
    std::ifstream test_data{test_file};//"../test_data_catboost.txt"};
    ...
    for (;;) {
        ...
        if (!read_features_csv(test_data, features)) break;
        auto probabilities = predictor.predict_proba_vector(features);
...
int main(int argc, char* argv[])
{
    if(argc != 3){
        ...
        std::string str(argv[0]);
        auto found=str.find_last_of("/\\");
        cout<<str.substr(found+1)<<"  "<<"path/to/test.csv  path/to/logreg_coef.txt"<<endl;
        cout<<"EXAMPLE:"<<endl;
